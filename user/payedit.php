...
if($_GET['my']=='edit') {
$id=intval($_GET['id']);
$row=$DB->get_row("SELECT * FROM auth_pays WHERE id='{$id}'and daili='{$daili_id}' limit 1");
...
$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".代理.$user."','修改易支付认证','".$date."','".$city."','IP：".$clientip."|QQ：".$qq."|域名：".$url."|名字：".$name."')");
...
?>
      <div class="panel panel-primary">
        <div class="panel-heading text-left"><h3 class="panel-title">编辑认证授权</h3></div>
