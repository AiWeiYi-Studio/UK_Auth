...
$newpass=($newpass);
$sql="update `auth_daili` set `user` ='{$user}',`pass` ='{$newpass}' where `uid`='{$udata['uid']}'";
if($DB->query($sql)){
	setcookie("admin_token", "", time() - 604800);
	@header('Content-Type: text/html; charset=UTF-8');
	$city=get_ip_city($clientip);
		$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$udata['user']."','修改密码','".$date."','".$city."','原密码".$oldpass."|新密码".$newpass."|原账号".$olduser."|新账号".$user."')");
