...
$row=$DB->get_row("SELECT * FROM auth_site WHERE url='{$url}' order by id desc limit 1");
if($row=='')exit("<script language='javascript'>alert('授权平台不存在该url！');history.go(-1);</script>");
if($row['active']==0)exit("<script language='javascript'>alert('此url的授权已被封禁！');history.go(-1);</script>");
        ...
        <div class="panel-body">
          <form action="./index.php" method="GET" class="form-horizontal" role="form">
            <div class="input-group">
              <span class="input-group-addon">授权的URL</span>
              <input type="text" name="url" value="<?=@$_GET['url']?>" class="form-control" placeholder="购买授权的url" autocomplete="off" autofocus="autofocus" required/>
            </div><br/>
