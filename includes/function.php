...
function pay_api($url) {
	global $DB;
	$row=$DB->get_row("SELECT * FROM auth_pays WHERE url='$url' limit 1");
	if ($row) {
...
{
	$query = curl_get("http://auth.ukyun.cn/api/check.php?url=".$_SERVER["HTTP_HOST"]."&authcode=".authcode."&ver=".VERSION);
...
function checkauthurl($url,$authcode) {
	global $DB,$date,$conf;
	//if(!$url && !$authcode)return false;
	$row=$DB->get_row("SELECT * FROM auth_site WHERE url='$url' limit 1");
	if ($row) {
