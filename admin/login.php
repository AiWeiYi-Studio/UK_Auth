...
if(isset($_POST['user']) && isset($_POST['pass'])){ 
	...
	if($row['user']=='') {
       ...
       exit ("<script language='javascript'>alert('登录失败，验证码错误！');history.go(-1)</script>");
		...
		$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".站长.$user."','登陆站长后台','".$date."','".$city."','IP:".$clientip."')");
	...
	@header('Content-Type: text/html; charset=UTF-8');
	$city=get_ip_city($clientip);
$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".站长.$user."','注销登录','".$date."','".$city."','".$clientip."')");
...
?>
<?php
if ($conf['adminloginmuban'] && file_exists("../template/login/{$conf['adminloginmuban']}/index.php")){
