...
<div class="form-group">
<label>站长权限:</label>
<select name="power" class="form-control"><option value="1">副站长</option><option value="2">合作商</option></select>
</div>
...
<div class="form-group">
<label>站长权限:</label>
<select name="power" class="form-control" default="'.$row['power'].'"><option value="1">副站长</option><option value="2">合作商</option></select>
</div>
...
$power=$_POST['power'];
$name=$_POST['name'];
if($user==NULL or $pwd==NULL or  $qq==NULL or $active==NULL or $power==NULL or $name==NULL){
showmsg('保存错误,请确保每项都不为空!',3);
} else {
$rows=$DB->get_row("select * from auth_user where user='$user' limit 1");
if($rows)
exit("<script language='javascript'>alert('站长名已存在！');window.location.href='user.php?my=add';</script>");
$sql="insert into `auth_user` (`user`,`pass`,`qq`,`active`,`power`,`name`,`boss`) values ('".$user."','".$pwd."','".$qq."','".$active."','".$power."','".$name."','".$udata['uid']."')";
if($DB->query($sql)){
...
{
$id=intval($_GET['id']);
$rows=$DB->get_row("select * from auth_user where uid='$id' limit 1");
if(!$rows)
	showmsg('当前记录不存在！',3);
$user=$_POST['user'];
$pwd=$_POST['pwd'];
...
$power=$_POST['power'];
$name=$_POST['name'];
if($user==NULL or $pwd==NULL or $active==NULL or $power==NULL or $name==NULL){
showmsg('保存错误,请确保每项都不为空!',3);
} else {
$city=get_ip_city($clientip);
$users=$udata['user'];
$DB->query("insert into `ukyun_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$users."','修改站长$user','".$date."','".$city."','IP:".$clientip."')");
if($DB->query("update auth_user set user='$user',pass='$pwd',qq='$qq',active='$active',power='$power',name='$name' where uid='{$id}'"))
...
{
$id=intval($_GET['id']);
$sql="DELETE FROM auth_user WHERE uid='$id'";
	$city=get_ip_city($clientip);
