...
<div class="form-group">
<label>用户权限:</label>
<select name="power" class="form-control" default="'.$row['power'].'">
<option value="1">超级授权商</option>
<option value="2">日站协助员</option>
<option value="3">易支付认证商</option>
<option value="4">域名授权商</option>
...
$sql="insert into `auth_daili` (`user`,`pass`,`qq`,`power`,`name`,`active`,`boss`) values ('".$user."','".$pwd."','".$qq."','".$power."','".$name."','".$active."','0')";
if($DB->query($sql)){
...
$con='系统共有 <b>'.$numrows.'</b> 个代理用户<br/><a href="./ulist.php?my=add" class="btn btn-primary">添加用户</a>&nbsp;<a href="#" data-toggle="modal" data-target="#search" id="search" class="btn btn-success">搜索</a>&nbsp;<a href="#" data-toggle="modal" data-target="#jieshao" id="jieshao" class="btn btn-success">等级介绍</a>';
...
if($res['power']==1){$p="超级授权商";}elseif($res['power']==2){$p="日站协助员";}elseif($res['power']==3){$p="易支付认证商";}elseif($res['power']==4){$p="域名授权商";}elseif($res['power']==5){$p="普通用户";}
