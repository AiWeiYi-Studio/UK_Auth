...
$uplog = 'http://auth.ukyun.cn/api/uplog.php';
$uplogs = file_get_contents($uplog); 
$notice = 'http://auth.ukyun.cn/api/notice.php';
$notices = file_get_contents($notice); 
$advertisement = 'http://auth.ukyun.cn/api/advert.php';
$advertisements = file_get_contents($advertisement); 
$ver = 'http://auth.ukyun.cn/api/ver.php';
$vers = file_get_contents($ver); 
$paycheck = 'http://auth.ukyun.cn/api/paycheck.php?url=http://'.$_SERVER['HTTP_HOST'].'/';
...
$logs=$DB->count("SELECT count(*) from auth_log");
$pays=$DB->count("SELECT count(*) from auth_pays");
$kms=$DB->count("SELECT count(*) from auth_kms");
if($udata['uid']==1){
$powers='全能站长';       
...
if(file_get_contents('http://auth.ukyun.cn/')){//服务器系统
$connect='<font color="green">连接正常</font> <a href="http://auth.ukyun.cn/" class="btn btn-xs btn-info">点此访问授权站</a>';
$_SESSION['connet']==true;//防止过多访问造成卡顿
}else{
$connect='<font color="red">连接失败</font> <a href="http://auth.ukyun.cn/" class="btn btn-xs btn-info">点此访问授权站</a>';
$webauth='1';
...
if($_SESSION['connet']!=true){
if(file_get_contents('https://api.ukyun.cn/webauth/')){//服务器系统
$connect3='<font color="green">连接正常</font> <a href="https://api.ukyun.cn/webauth/" class="btn btn-xs btn-info">点此访问资源站</a>';
$_SESSION['connet']==true;//防止过多访问造成卡顿
}else{
$connect3='<font color="red">连接失败</font> <a href="https://api.ukyun.cn/webauth/" class="btn btn-xs btn-info">点此访问资源站</a>';
$ukyun1==1;
$_SESSION['connet']==true;//防止过多访问造成卡顿 直接结束
}
}
...
if($_SESSION['update']!=true){
	$update=file_get_contents('http://auth.ukyun.cn/api/check.php?url='.$_SERVER['HTTP_HOST']."&authcode=".$authcode."&ver=".VERSION);//获得json返回信息
	$url='http://auth.ukyun.cn/api/check.php?url='.$_SERVER['HTTP_HOST']."&authcode=".$authcode."&ver=".VERSION;
...
<ul class="list-group">
<li class="list-group-item"><span class="glyphicon glyphicon-stats"></span> <b>统计：</b>正版:<?=$sites?>,盗版:<?=$blocks?>,认证:<?=$pays?>,卡密:<?=$kms?>,站长数:<?=$admins?>,用户数:<?=$users?>,日志:<?=$logs?>,</li>
...
<li class="list-group-item"><b>程序当前版本：</b><?php echo NEWVERSION ?></li>
...
<?php
if(phpversion()>5.6){
echo '<li class="list-group-item"><span class="btn-sm btn-danger">危险</span>&nbsp;PHP大于5.6会导致盗版入库失效</li>';
}
?>
<?php
...
?>
<?php
if($udata['user']!==$udata['pass'] and $dbconfig['user']!==$dbconfig['pwd'] and phpversion()<5.7){
