...
<input type="text" class="form-control" name="uid" placeholder="请输入代理id"><br/>
...
if(isset($_GET['qq'])) {
	$qq=intval($_GET['qq']);
	$sql=" `qq`='{$qq}'";
	$gls=$DB->count("SELECT count(*) from auth_pays WHERE{$sql}");
	...
	$sql=" `daili`='{$uid}'";
	$gls=$DB->count("SELECT count(*) from auth_pays WHERE{$sql}");
	$con='代理用户(UID:'.$uid.')共有 <b>'.$gls.'</b> 个域名';
	$link='&id='.$_GET['uid'];
}else{
	$gls=$DB->count("SELECT count(*) from auth_pays WHERE 1");
	$sql=" 1";
	$con='认证平台共有 <b>'.$gls.'</b> 个域名<br/><a href="./addpay.php" class="btn btn-primary">添加认证</a>&nbsp;<a href="#" data-toggle="modal" data-target="#search" id="search" class="btn btn-success">搜索ID</a>&nbsp;<a href="#" data-toggle="modal" data-target="#searchqq" id="searchqq" class="btn btn-success">搜索QQ</a>';
}
}
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
<?php
$rs=$DB->query("SELECT * FROM auth_pays WHERE{$sql} order by id desc limit $pageu,$pagesize");
while($res = $DB->fetch($rs))
{
echo '<tr><td>'.$res['id'].'</td><td><a href="http://wpa.qq.com/msgrd?v=3&uin='.$res['qq'].'&site=qq&menu=yes">'.$res['qq'].'</a></td><td>'.$res['name'].'</td><td><a href="../api/jump.php?url='.urlencode(''.$res['url'].'').'" target="_blank">'.$res['url'].'</a></td><td>'.$res['date'].'</td><td>'.($res['active']==1?'<font color=green>正常</font>':'<font color=red>已封禁</font>').'</td><td><a href="./payedit.php?my=edit&id='.$res['id'].'" class="btn btn-xs btn-info">编辑</a> <a href="./payedit.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此条授权记录吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
