...
function ptuiCB(code,uin,sid,skey,pskey,pskey2,nick){
	...
	switch(code){
		case '0':
			$('#login').html('<div class="alert alert-success">QQ验证成功！'+decodeURIComponent(nick)+'</div><br/><a href="../api/download_get.php?my=installer&qq='+uin+'" target="_blank" class="btn btn-primary">完整安装包</a>&nbsp;<a href="../api/download_get.php?my=updater&qq='+uin+'" target="_blank" class="btn btn-success">更新包</a><hr/>提示：如果需要全新搭建或之前未搭建过，请下载完整安装包；如果之前搭建过，请下载更新包直接覆盖，数据不会丢失。');
			$('#qrimg').hide();
			$('#submit').hide();
			$('#mobile').hide();
...
function mloginurl(){
	...
	$('#mlogin').html("正在跳转...");
	$.post("get/qrcode.php?r="+Math.random(1),"image="+encodeURIComponent(imagew), function(arr) {
		if(arr.code==0) {
